    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">DHCP Servers</h2>
            <p className="text-gray-600 mt-1">Manage Kea DHCPv4/DHCPv6 servers, pools, and leases</p>
          </div>
        </div>
    ...
    <div>
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-6">
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">DHCP Servers</h2>
            <p className="text-gray-600 mt-1">Address pools, reservations, custom options, and lease control</p>
          </div>
          <InstanceSelector />
        </div>
        <button
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="divide-y divide-gray-100">
            {servers && servers.length > 0 ? servers.map((server) => (
              ...
              <div className="p-4 text-sm text-gray-500">No DHCP servers yet.</div>
            )}
          </div>
        </div>
            ...
            <div className="divide-y divide-gray-100">
              {subnets && subnets.length > 0 ? subnets.map((subnet) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-gray-50 ${selectedSubnet?.id === subnet.id ? 'bg-primary-50' : ''}`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium text-gray-900 truncate">{subnet.name}</span>
                    <span className="text-xs px-2 py-0.5 rounded bg-gray-100 text-gray-700 uppercase">{subnet.type}</span>
                  </div>
                  <div className="mt-1 text-xs text-gray-500 break-all">{subnet.subnet}</div>
                  <div className="mt-1 text-xs text-gray-500">{subnet.pools_count} pools · {subnet.reservations_count} reservations</div>
                  <div className="mt-2 flex items-center gap-2">
                    <button
                      onClick={(event) => {
                        event.stopPropagation()
                ...
                <div className="p-4 text-sm text-gray-500">No subnets for this server.</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="max-h-[480px] overflow-y-auto">
            <div className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase">Pools</div>
              ...
              <div className="divide-y divide-gray-100">
                {pools && pools.length > 0 ? pools.map((pool) => (
                  <div key={pool.id} className="px-4 py-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium text-gray-900 break-all">{pool.start_address} - {pool.end_address}</span>
                      <button
                ...
                )) : <div className="px-4 pb-3 text-sm text-gray-500">No pools configured.</div>}
              </div>
            )}

            <div className="px-4 py-2 mt-2 text-xs font-semibold text-gray-500 uppercase border-t border-gray-100">Reservations</div>
              ...
              <div className="divide-y divide-gray-100">
                {reservations && reservations.length > 0 ? reservations.map((reservation) => (
                  <div key={reservation.id} className="px-4 py-3">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <div className="text-sm font-medium text-gray-900 break-all">{reservation.ip_address}</div>
                        <div className="text-xs text-gray-500 break-all">{reservation.hw_address}</div>
                      </div>
                      <button
                ...
                )) : <div className="px-4 pb-3 text-sm text-gray-500">No reservations configured.</div>}
              </div>
            )}

            <div className="px-4 py-2 mt-2 text-xs font-semibold text-gray-500 uppercase border-t border-gray-100 flex items-center justify-between">
              <span>Options</span>
              <button
                disabled={!selectedSubnet}
              ...
              <div className="divide-y divide-gray-100">
                {options && options.length > 0 ? options.map((option) => (
                  <div key={option.id} className="px-4 py-3">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{option.option_name} ({option.option_code})</div>
                        <div className="text-xs text-gray-500 break-all">{option.option_value}</div>
                      </div>
                      <button
                ...
                )) : <div className="px-4 pb-3 text-sm text-gray-500">No custom options configured.</div>}
              </div>
            )}
          </div>
        </div>
            ...
            {leasesLoading ? <div className="p-4"><LoadingSpinner /></div> : (
              leases && leases.length > 0 ? leases.map((lease) => (
                <div key={lease.id} className="px-4 py-3">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <div className="text-sm font-medium text-gray-900 break-all">{lease.ip_address}</div>
                      <div className="text-xs text-gray-500 break-all">{lease.hostname || lease.hw_address || lease.client_id || 'n/a'}</div>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded ${lease.state === 'active' ? 'bg-green-100 text-green-700' : lease.state === 'released' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      {lease.state}
                    </span>
                  </div>
                  {lease.state === 'active' ? (
                    <button
                      ...
                      className="mt-2 text-xs px-2 py-1 rounded bg-amber-100 text-amber-800 hover:bg-amber-200"
                    >
                      Release lease
                    </button>
                  ) : null}
                </div>
              )) : <div className="p-4 text-sm text-gray-500">No leases for this subnet.</div>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      {haEnabled ? (
        <>
          <label className="block">
            <span className="text-sm font-medium text-gray-700">HA Mode</span>
            <select value={haMode} onChange={(event) => setHaMode(event.target.value as DHCPHAMode)} className="mt-1 w-full border rounded-lg px-3 py-2">
              <option value={DHCPHAMode.HOT_STANDBY}>Hot standby</option>
              <option value={DHCPHAMode.LOAD_BALANCING}>Load balancing</option>
            </select>
          </label>
          <label className="block">
            <span className="text-sm font-medium text-gray-700">HA Peer Address</span>
            <input value={haPeerAddress} onChange={(event) => setHaPeerAddress(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" placeholder="10.0.0.2" />
          </label>
        </>
      ) : null}
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="block">
          ...
          <select value={type} onChange={(event) => setType(event.target.value as DHCPSubnetType)} className="mt-1 w-full border rounded-lg px-3 py-2">
      ...
      <label className="block">
        <span className="text-sm font-medium text-gray-700">Subnet CIDR</span>
        <input value={subnet} onChange={(event) => setSubnet(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">Routers (comma separated)</span>
        <input value={routers} onChange={(event) => setRouters(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">DNS servers (comma separated)</span>
        <input value={dnsServers} onChange={(event) => setDnsServers(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <div className="grid grid-cols-3 gap-3">
        <label className="block">
          <span className="text-xs font-medium text-gray-700">Min</span>
          <input type="number" value={leaseMin} onChange={(event) => setLeaseMin(Number(event.target.value))} className="mt-1 w-full border rounded-lg px-3 py-2" />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-700">Default</span>
          <input type="number" value={leaseDefault} onChange={(event) => setLeaseDefault(Number(event.target.value))} className="mt-1 w-full border rounded-lg px-3 py-2" />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-700">Max</span>
          <input type="number" value={leaseMax} onChange={(event) => setLeaseMax(Number(event.target.value))} className="mt-1 w-full border rounded-lg px-3 py-2" />
        </label>
      </div>
      {type === DHCPSubnetType.V6 ? (
        <label className="block">
          <span className="text-sm font-medium text-gray-700">Delegated Prefix Length (optional)</span>
          <input value={delegatedPrefixLength} onChange={(event) => setDelegatedPrefixLength(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" placeholder="64" />
        </label>
      ) : null}
      <div className="flex justify-end">
        <button disabled={loading} className="px-4 py-2 rounded-lg bg-primary-600 text-white disabled:opacity-50">
...
function DHCPPoolForm({
  ...
  return (
    ...
    >
      <label className="block">
        <span className="text-sm font-medium text-gray-700">Start Address</span>
        <input value={startAddress} onChange={(event) => setStartAddress(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">End Address</span>
        <input value={endAddress} onChange={(event) => setEndAddress(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <div className="flex justify-end">
        <button disabled={loading} className="px-4 py-2 rounded-lg bg-primary-600 text-white disabled:opacity-50">
  ...
  return (
    ...
    >
      <label className="block">
        ...
        <input value={hostname} onChange={(event) => setHostname(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">IP Address</span>
        <input value={ipAddress} onChange={(event) => setIpAddress(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">{subnetType === DHCPSubnetType.V4 ? 'MAC Address' : 'DUID'}</span>
        <input value={hwAddress} onChange={(event) => setHwAddress(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <div className="flex justify-end">
        <button disabled={loading} className="px-4 py-2 rounded-lg bg-primary-600 text-white disabled:opacity-50">
...
function DHCPOptionForm({
  ...
  return (
    ...
    >
      <label className="block">
        <span className="text-sm font-medium text-gray-700">Option Code</span>
        <input type="number" value={optionCode} onChange={(event) => setOptionCode(Number(event.target.value))} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">Option Name</span>
        <input value={optionName} onChange={(event) => setOptionName(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <label className="block">
        <span className="text-sm font-medium text-gray-700">Option Value</span>
        <input value={optionValue} onChange={(event) => setOptionValue(event.target.value)} className="mt-1 w-full border rounded-lg px-3 py-2" />
      </label>
      <div className="flex justify-end">
        <button disabled={loading} className="px-4 py-2 rounded-lg bg-primary-600 text-white disabled:opacity-50">
